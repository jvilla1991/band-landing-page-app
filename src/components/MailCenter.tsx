...
/* Demo data — the real app gets a DM backend; until then the Mail Center
   is a local-state walkthrough of the request -> accept -> thread flow. */
interface DmRequest {
  id: string
  user: string
  date: string
...
type Note = { k: 'ok' | 'dim'; t: string } | null
...
function MailCenter() {
  ...
  function openMsg(m: MailMsg) {
    setOpenId(m.id)
    setMessages((ms) => ms.map((x) => (x.id === m.id ? { ...x, unread: false } : x)))
  }

  function revoke(m: MailMsg) {
    setOpenId(null)
